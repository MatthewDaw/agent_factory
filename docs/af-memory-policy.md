...
- **tag match** — active `category="check"` facts whose `meta.applies_to` (array, supports `"*"`)
  matches any of the ticket's tags (`meta.tags` / `meta.applies_to`); via `facts_by`.
- **surface match** — active checks bound (via the `renders` edge) to any surface the ticket renders
  (`meta.surfaces` / `meta.screen_ids`); via `surface_checks` → `/surfaces/{screen}/checks`.

A semantic lane (embedding the check predicate against ticket text) is a documented TODO, not in v1.
Checks are **declarative + read-only during builds** — a check owns its own applicability predicate
and is edited only on explicit user request, never as a side effect of building.
...
get_fact(cid) -> dict
facts_by(category=None, meta=None, state="active") -> list[dict]
patch_meta(cid, meta_dict) -> dict          # MERGE meta (build_state / claim / pinned_checks)
record_outcome(cid, success) -> dict
surface_checks(project, screen_id, scope=None) -> list[dict]
ping() -> bool
```
...
```python
resolve_checks(ticket, project="") -> list[dict]                   # the QUERY (tag ∪ surface)
...
start_ticket(cid, owner, project="", ttl=900) -> list[dict]|None   # claim + resolve + pin
```
