...
memory operation, and again after every `/mcp` reconnect or Praxis restart** — not just once per
session. If the active org is not `agent-factory`, run `praxis_select_org("agent-factory")`. Never
write into another org.
