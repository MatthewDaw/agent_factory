...
def facts_by(category: str | None = None, meta: dict | None = None,
    ...
    (pass ``"any"`` to span all lifecycle states)."""
    params: dict[str, Any] = {"state": state}
    if category is not None:
        params["category"] = category
    if meta:
        params["meta"] = json.dumps(meta)
    out = _request("GET", "/facts/by", params=params)
...
def surface_checks(project: str, screen_id: str, scope: str | None = None) -> list[dict]:
